...
test_constructor <- function()
{
   ...
   checkTrue(all(c("TrenaProjectBrainCell", "TrenaProjectHG38") %in% is(tpl)))
   checkEquals(getFootprintDatabasePort(tpl), 5432)
...
{
   expected <- "Micro_TYROBP"
   checkTrue(all(expected %in% getExpressionMatrixNames(tpl)))

   mtx <- getExpressionMatrix(tpl, expected)
   checkEquals(dim(mtx), c(14340, 264))
   checkEquals(head(sort(rownames(mtx)), n=3), c("A1BG", "A1CF", "A2M"))

   checkTrue(max(mtx) < 6800)
...
{
   printf("--- test_buildSingleGeneModel_TREM2")
   ...
   checkTrue("TGIF1" %in% head(tbl.model$gene))
   checkTrue(max(tbl.model$pearsonCoeff) > 0.5)
     # a modest sanity check on pearsonCoeff: should be exactly what we see in the expression matrix
   checkEqualsNumeric(cor(mtx["TREM2",], mtx["TGIF1",]), subset(tbl.model, gene=="TGIF1")$pearsonCoeff)
